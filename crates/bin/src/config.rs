...
pub struct Config {
    ...
    /// Postgres connection pool size. `FERRUM_DB_MAX_CONNECTIONS`, default 10.
...
impl Config {
    pub fn from_env() -> Result<Self> {
        ...
        let db_max_connections = std::env::var("FERRUM_DB_MAX_CONNECTIONS")
            .ok()
            .and_then(|s| s.parse::<u32>().ok())
            .filter(|n| *n > 0)
            .unwrap_or(10);
        let jwt_secret =
            std::env::var("FERRUM_JWT_SECRET").context("FERRUM_JWT_SECRET must be set")?;
        ...
        let jwt_ttl_secs = std::env::var("FERRUM_JWT_TTL_SECS")
            ...
            .unwrap_or(86400);
        let bind = std::env::var("FERRUM_BIND").unwrap_or_else(|_| "0.0.0.0:8080".into());
        let log = std::env::var("FERRUM_LOG").unwrap_or_else(|_| "info".into());
        let page_size_max = std::env::var("FERRUM_PAGE_SIZE_MAX")
            .ok()
            .and_then(|s| s.parse::<u32>().ok())
            .unwrap_or(100);
        let studio_dir = std::env::var("FERRUM_STUDIO_DIR")
            .ok()
            .filter(|s| !s.is_empty());
        ...
        let docs_enabled = std::env::var("FERRUM_DOCS_ENABLED")
            ...
            .unwrap_or(true);
        let api_version = std::env::var("FERRUM_API_VERSION").unwrap_or_else(|_| "0.1.0".into());
        let public_base_url = std::env::var("FERRUM_PUBLIC_URL").unwrap_or_else(|_| "/".into());
        Ok(Self {
            database_url,
            db_max_connections,
            jwt_secret,
...
mod tests {
    ...
    fn db_max_connections_parses() {
        ...
        // FERRUM_JWT_SECRET and tests run on parallel threads).
