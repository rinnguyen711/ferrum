...
| [progress.md](progress.md) | done, in progress, todo, current work, features shipped, filters, pagination, bulk actions, builder save UX, custom roles, role editor, user detail, confirmed/blocked, GraphQL surface, shared content CRUD fns, roadmap status, audit log, AuditSink, audit_log table, who changed what, prune worker, reqctx middleware, schema-as-code, TOML sync, RUSTAPI_SCHEMA_DIR, sync_from_path, plan_sync, plan_components, managed types, component sync, seed removed |
| [decisions.md](decisions.md) | trade-offs, why X over Y, rationale, rejected approaches, server-side filtering, floating save bar, tab counts, no UI test infra, per-type permissions, role cache vs query, system roles locked, user field scope, GraphQL dynamic schema, GqlRegistry, relation scalar ids, async-graphql-axum pin |
| [gotchas.md](gotchas.md) | bugs, edge cases, surprises, footguns, PATCH vs PUT, test setup, dev creds, compose stack, loader vs popovers, design CSS porting, sqlx migrate rebuild, test flake, tests in bin/tests, AppState construction sites, GraphQL relation dangling type, gql 503, axum 0.7 vs 0.8, REST vs GraphQL side-effect blind spot, CSV formula injection, two settings sidebars, resolved_options drops keys, schema sync idempotency, background subagent commit/scope |
