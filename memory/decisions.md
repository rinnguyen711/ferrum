...
## Name "rustapi" collides with existing OSS — RESOLVED, renamed to Ferrum (2026-07-09)
- Old name "rustapi" clashed with an existing OSS project. **Chose Ferrum** despite `ferrum` being flagged as taken on crates.io during the 2026-06-15 candidate search (Ferralis was the earlier top pick; user overrode). Display form `Ferrum`; lowercase `ferrum` for binary/crate-suffix/db/image.
- Shipped on `main`, commit 55c666a: crates `ferrum-core/sql/schema/http/media`, binary `ferrum`, env `FERRUM_*`, DB name/role `ferrum`, docs title (book.toml + intro), README, Docker/CI, UI pkg `ferrum-ui`, `book/theme/rustapi.css` -> `ferrum.css`. Verified: build/clippy/test (145 passed), typecheck, mdbook.
- Repo: github.com/rinnguyen711/ferrum (pushed). NOTE the `bench/rustapi-vs-strapi` branch was deliberately NOT renamed — it stays isolated (see gotchas + bench-branch decision below).
- Breaking for live deploys: `FERRUM_*` + `ferrum` DB replace `RUSTAPI_*`/`rustapi`.
...
## Keyset pagination (2026-06-16, MERGED to main, merge 240bab5)
- **Additive cursor path, kept offset.** Added `?cursor=` keyset alongside existing `?page=`/`?pageSize=` OFFSET — chosen over replacing offset (breaks admin UI page-jump + any ?page= client) or hybrid auto-switch (server can't tell sequential-vs-jump from stateless ?page=N). Matches Strapi/Contentful (kept offset, added cursor) + Sanity (keyset on unique _id). Non-breaking: offset clients untouched.
- **`?cursor=first` sentinel starts keyset.** First keyset page has no token, so couldn't distinguish "start keyset" from plain offset. Rejected: making offset full-pages emit nextCursor (contradicts spec + caused 500 on non-scalar sort). `cursor=first` → keyset mode, after=None. `keyset_mode = cursor.is_some()`. Offset mode stays pure (no nextCursor, no read_sort_value call).
- **Cursor key = (sort_col, id) tiebreak.** id (UUID PK) appended to ORDER BY + row-value seek so ordering is total even when sort col has dup values — prevents seam skip/dup. Chosen over id-only (can't keyset arbitrary user sorts) or opaque multi-col encode (YAGNI — sort is single-column).
- **Opaque base64 token** (`{c:sortcol, d:dir, v:jsonval, i:uuid}`), validated against request sort (col+dir mismatch → 400). Lets internals change without breaking clients.
- **Count opt-out by mode.** withCount defaults: cursor mode → false (keyset pages till empty), offset → true. `?withCount=false` skips the full-table count(*) scan. Keyset meta = {pageSize, nextCursor}; offset meta = {page, pageSize, total?}.
- **No deep-OFFSET cap.** Considered Contentful-style hard cap (reject ?page= past N) — rejected, would break clients paging deep slowly. Deep offset stays reachable (slow, never errors); cursor is the opt-in fast path.
- Spec/plan: docs/superpowers/{specs,plans}/2026-06-16-keyset-pagination*.

## Bench stays on its own branch (2026-06-17)
**Keep `bench/rustapi-vs-strapi` as a long-lived separate branch — do NOT merge bench/ to main, do NOT delete the branch.** User wants it isolated but preserved ("might go back later"). It's 79 commits behind main + has its own divergent bits (old tracked memory/, a `seed` config field). To run the bench against current code, merge main INTO the bench branch (working tree builds from `build: context: ..`), update k6, run there — never the reverse. main stays bench-free. Don't re-raise "fold bench onto main / retire stale branch" — decided against.
